<template>
  <div class="snes">
    <div class="left">
      <div class="pad">
        <div class="control top">
          <button class="arrow" @click="performAction('top')"></button>
        </div>
        <div class="control left">
          <button class="arrow" @click="performAction('left')"></button>
        </div>
        <div class="control right">
          <button class="arrow" @click="performAction('right')"></button>
        </div>
        <div class="control bottom">
          <button class="arrow" @click="performAction('bottom')"></button>
        </div>
    ...
    <div class="middle">
      ...
      <div class="title"></div>
      <button class="select" @click="performAction('select')"></button>
      <button class="start" @click="performAction('start')"></button>
    </div>
    <div class="right">
      <div class="circle">
        <div class="group-1">
          <button class="green" @click="performAction('y')"></button>
          <button class="blue" @click="performAction('x')"></button>
        </div>
        <div class="group-2">
          <button class="yellow" @click="performAction('b')"></button>
          <button class="red" @click="performAction('a')"></button>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      performAction(action) {
        ...
        this.manageWebSocket(action);
      },

      manageWebSocket(action) {
        switch (action) {
          case 'top' :
            this.socket.send(JSON.stringify({ action: 'MOVE_FORWARD' }));
            break;
          case 'bottom':
            this.socket.send(JSON.stringify({ action: 'MOVE_BACKWARD' }));
            break;
          default:
      ...
      connectWS() {
        ...
        this.socket.onopen = (data) => {
          window.console.log('onOpen : ', 'color:green', data);
          this.socket.onmessage = (response) => {
            window.console.log('onMessage : ', 'color:blue', response);
          };
        };
      },
    },
...
<style lang="stylus" scoped>
  ...
  .snes:after
    ...
    box-shadow: inset -6px -17px 5px rgba(0, 0, 0, .2)
  ...
  .snes > .left .pad
    ...
    background: $bg_elements
    border-radius: 50%
    box-shadow: inset 0 0 1px rgba(120, 120, 120, .6),
  ...
  .snes > .left .pad .middle:after
    ...
    width: 70%
    border-radius: 100%
    box-shadow: inset 0 0 2px rgba(120, 120, 120, .6),
  ...
  .snes > .middle button
    ...
    box-shadow: inset 0 0 1px rgba(120, 120, 120, .7),
  ...
  .snes > .right .circle .group-1:before,
  .snes > .right .circle .group-2:before
    box-shadow: -2px -1px 0 0px rgba(0, 0, 0, .2),
  ...
  .snes > .right .circle button
    ...
    box-shadow: inset 0 -1px 0 4px rgba(0, 0, 0, .35),
  ...
  .snes > .right .circle button:active
    box-shadow: inset 0 -1px 0 5px rgba(0, 0, 0, .25),
      inset 0 0 0 1px rgba(0, 0, 0, .7),
  ...
  /* Overlap box shadow of the group pseudos */
